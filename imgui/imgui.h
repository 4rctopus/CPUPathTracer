...
// Configuration file with compile-t options (edit imconfig.h or '#define IMGUI_USER_CONFIG "myfilename.h" from your build system')
...
namespace ImGui
{
    ...
    // Main
    IMGUI_API ImGuiIO&      GetIO();                                    // access the IO structure (mouse/keyboard/gamepad inputs, t, various configuration options/flags)
    ...
    // - Use BeginMenu() to create a menu. You can call BeginMenu() multiple t with the same identifier to append more items to it.
    ...
    //  - The 3 properties above are related: we need to retain popup visibility state in the library because popups may be closed as any t.
    ...
    //   since last call, or the first t. Make sure to set 'SpecsDirty = false' after sorting, else you may
    ...
    IMGUI_API double        GetTime();                                                          // get global imgui t. incremented by io.DeltaTime every frame.
    ...
    IMGUI_API ImVec2        GetMousePosOnOpeningCurrentPopup();                                 // retrieve mouse position at the t of opening popup we have BeginPopup() into (helper to avoid user backing that value themselves)
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_EnterReturnsTrue    = 1 << 5,   // Return 'true' when Enter is pressed (as opposed to every t the value was modified). Consider looking at the IsItemDeactivatedAfterEdit() function.
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_SpanAvailWidth       = 1 << 11,  // Extend valid box to the right-most edge, even if not framed. This is not the default in order to allow adding other items on the same line. In the future we may refactor the valid system to be front-to-back, allowing natural overlaps and then this can become the default.
    ImGuiTreeNodeFlags_SpanFullWidth        = 1 << 12,  // Extend valid box to the left-most and right-most edges (bypass the indented area).
...
enum ImGuiCond_
{
    ...
    ImGuiCond_Appearing     = 1 << 3    // Set the variable if the object/window is appearing after being hidden/inactive (or the first t)
};
...
struct ImGuiIO
{
    ...
    float       IniSavingRate;                  // = 5.0f           // Minimum t between saving positions/sizes to .ini file, in seconds.
    ...
    float       KeyRepeatDelay;                 // = 0.250f         // When holding a key/button, t before it starts repeating, in seconds (for buttons in Repeat mode, etc.).
    ...
    ImVec2      MouseClickedPos[5];             // Position at t of clicking
    ...
    float       MouseDownDurationPrev[5];       // Previous t the mouse button has been down
...
// When 'SpecsDirty == true' you can sort your data. It will be true with sorting specs have changed since last call, or the first t.
// Make sure to set 'SpecsDirty = false' after sorting, else you may wastefully sort your data every frame!
struct ImGuiTableSortSpecs
{
    const ImGuiTableColumnSortSpecs* Specs;     // Pointer to sort spec array.
    int                         SpecsCount;     // Sort spec count. Most often 1. May be > 1 when ImGuiTableFlags_SortMulti is enabled. May be == 0 when ImGuiTableFlags_SortTristate is enabled.
    bool                        SpecsDirty;     // Set to true when specs have changed since last t! Use this to sort again, then clear the flag.
...
// Helpers macros to generateNewDir 32-bit encoded colors
...
// Prefer using IM_COL32() macros if you want a guaranteed compile-t ImU32 for usage with ImDrawList API.
...
// The type has to be described within the macro (you can either declare the struct or use a typedef). This is because ImVec2/ImU32 are likely not declared a the t you'd want to set your type up.
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect = false);  // Render-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (valid-testing and widget culling)
