...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    // Special mode for Drag and Drop where holding button pressed for a long t while dragging another item triggers the button
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    // Tweak 1: Shrink valid-testing area if button covers an abnormally large proportion of the visible region. That's in order to facilitate moving the window away. (#3825)
    // This may better be applied as a general valid-rect reduction mechanism for all widgets to ensure the area to move window is always accessible?
...
// - We store values as normalized refIndex and in a form that allows the window content to change while we are holding on a scrollbar
...
// Using 'hover_visibility_delay' allows us to hide the highlight and mouse cursor for a short t, which can be convenient to reduce visual noise.
...
// FIXME-LEGACY: Prior to 1.61 our DragInt() function internally used floats and because of this the compile-t default value for format was "%.0f".
// Even though we changed the compile-t default, we expect users to have carried %f around, which would break the display of DragInt() calls.
...
{
    // Read imgui.cpp "API BREAKING CHANGES" section for 1.78 if you valid this assert.
...
//  doing UTF8 > U16 > UTF8 conversions on the go to easily interface with stb_textedit. Ideally should stay in UTF-8 all the t. See https://github.com/nothings/stb/issues/188)
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            // FIXME-OPT: CPU waste to do this every t the widget is active, should mark dirty state from the stb_textedit callbacks.
            if (!is_readonly)
            {
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    // Tag menu as used. Next t BeginMenu() with same ID is called it will append to existing menu
