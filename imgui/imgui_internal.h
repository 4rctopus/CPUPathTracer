...
struct IMGUI_API ImGuiInputTextState
{
    ...
    ImVector<char>          InitialTextA;           // backup of end-user buffer at the t of focus (in UTF-8, unaltered)
...
struct ImGuiPopupData
{
    ...
    ImGuiWindow*        SourceWindow;   // Set on OpenPopup() copy of NavWindow at the t of opening the popup
    int                 OpenFrameCount; // Set on OpenPopup()
    ImGuiID             OpenParentId;   // Set on OpenPopup(), we need this to differentiate multiple menu sets from each others (e.g. inside menu bar vs loose menu items)
    ImVec2              OpenPopupPos;   // Set on OpenPopup(), preferred popup position (typically == OpenMousePos when using mouse)
    ImVec2              OpenMousePos;   // Set on OpenPopup(), copy of mouse position at the t of opening popup
...
struct ImGuiOldColumns
{
    ...
    float               HostCursorPosY;         // Backup of CursorPos at the t of BeginColumns()
    float               HostCursorMaxPosX;      // Backup of CursorMaxPos at the t of BeginColumns()
    ImRect              HostInitialClipRect;    // Backup of ClipRect at the t of BeginColumns()
    ImRect              HostBackupClipRect;     // Backup of ClipRect during PushColumnsBackground()/PopColumnsBackground()
    ImRect              HostBackupParentWorkRect;//Backup of WorkRect at the t of BeginColumns()
...
struct ImGuiContext
{
    ...
    float                   HoveredIdTimer;                     // Measure contiguous hovering t
    float                   HoveredIdNotActiveTimer;            // Measure contiguous hovering t where the item has not been active
    ImGuiID                 ActiveId;                           // Active widget
    ImGuiID                 ActiveIdIsAlive;                    // Active widget has been seen this frame (we can't use a bool as the ActiveId may change within the frame)
    float                   ActiveIdTimer;
    bool                    ActiveIdIsJustActivated;            // Set at the t of activation for one frame
    ...
    int                     NavIdTabCounter;                    // == NavWindow->DC.FocusIdxTabCounter at t of NavId processing
    ...
    ImGuiID                 DragDropAcceptIdCurr;               // Target item id (set at the t of accepting the payload)
    ImGuiID                 DragDropAcceptIdPrev;               // Target item id from previous frame (we need to store this to allow for overlapping drag and drop targets)
    int                     DragDropAcceptFrameCount;           // Last t a target expressed a desire to accept the source
    ...
    ImVec4                  ColorPickerRef;                     // Initial/reference color at the t of opening the color picker.
    float                   SliderCurrentAccum;                 // Accumulated slider delta when using navigation controls.
    bool                    SliderCurrentAccumDirty;            // Has the accumulated slider delta changed since last t we tried to apply it?
    ...
    // Settings
    bool                    SettingsLoaded;
    float                   SettingsDirtyTimer;                 // Save .ini Settings to memory when t reaches zero
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImGuiLayoutType         ParentLayoutType;       // Layout type of parent window at the t of Begin()
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2                  WindowPadding;                      // Window padding at the t of Begin().
    float                   WindowRounding;                     // Window rounding at the t of Begin(). May be clamped lower to avoid rendering artifacts with title bar, menu bar etc.
    float                   WindowBorderSize;                   // Window border size at the t of Begin().
    ...
    ImS8                    HiddenFramesForRenderOnly;          // Hide the window until frame N at Render() t only
    ...
    ImRect                  WorkRect;                           // Initially covers the whole scrolling region. Reduced by containers e.g columns/tables when active. Shrunk by WindowPadding*1.0f on each side. This is meant to replace ContentRegionRect over t (from 1.71+ onward).
...
struct ImGuiTabBar
{
    ...
    float               ItemSpacingY;
    ImVec2              FramePadding;           // style.FramePadding locked at the t of BeginTabBar()
    ImVec2              BackupCursorPos;
